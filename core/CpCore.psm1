...
Import-Module -Force -DisableNameChecking (Join-Path $here 'Parsing.psm1') -ErrorAction SilentlyContinue
...
function __CP_GetModules {
  ...
  foreach ($catDir in Get-ChildItem -Path $modsRoot -Directory) {
    ...
    foreach ($psm1 in (Get-ChildItem -Path $catDir.FullName -Filter *.psm1 -File -EA SilentlyContinue)) {
      ...
      $accum += [pscustomobject]@{ Name=$name; Category=$catDir.Name; Path=$psm1.FullName; Priority=$prio; Meta=$meta }
    }
  ...
  # Force array semantics even for a single item
  $sorted = @($accum | Sort-Object Priority, Category, Name)
  return $sorted
}
...
Export-ModuleMember -Function Invoke-CpAutoCore
